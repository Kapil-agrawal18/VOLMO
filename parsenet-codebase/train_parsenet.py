...
for e in range(config.epochs):
    ...
    for train_b_id in range(config.num_train // config.batch_size):
        ...
        for _ in range(num_iter):
            ...
            primitives = torch.from_numpy(
                primitives.astype(np.int64))  # .cuda()
            if if_normals:
                input = torch.cat([points, normals], 2)
                embedding, primitives_log_prob, embed_loss = model(
                    input.permute(0, 2, 1), torch.from_numpy(labels), True
                )
            else:
                embedding, primitives_log_prob, embed_loss = model(
                    points.permute(0, 2, 1), torch.from_numpy(labels), True
                )
        ...
        log_value("iou", iou, train_b_id + e *
                  (config.num_train // config.batch_size))
        log_value(
            "embed_loss",
            embed_losses,
