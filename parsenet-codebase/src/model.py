...
def knn(x, k):
    ...
    with torch.no_grad():
        ...
        for b in range(batch_size):
            # torch.matmul used for matrix multipication
            inner = -2 * torch.matmul(x[b:b + 1].transpose(2, 1), x[b:b + 1])
            xx = torch.sum(x[b:b + 1] ** 2, dim=1,
                           keepdim=True)  # sum along column
...
def get_graph_feature(x, k=20, idx=None):
    batch_size = x.size(0)
    num_points = x.size(2)
    x = x.contiguous()  # a tensor whose elements are stored in a contiguous order without leaving any empty space between them
...
class DGCNNControlPoints(nn.Module):
    def __init__(self, num_control_points, num_points=40, mode=0):
        # num_control_points=20 , num_points=10, mode=0
        """
