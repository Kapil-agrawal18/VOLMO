    ...
    def __init__(self, path, batch_size, size_u=20, size_v=20, splits={}, closed=False):
        ...
        with h5py.File(path, "r") as hf:
            ...
            control_points = np.array(
                hf.get(name="controlpoints")).astype(np.float32)

        np.random.seed(0)
        List = np.arange(points.shape[0])  # 90296
            ...
            for batch_id in range(self.train_size // self.batch_size - 1):
                ...
                for i in range(self.batch_size):
                    ...
                    if align_canonical:
                        ...
                        R = self.rotation_matrix_a_to_b(
                            smallest_ev, np.array([1, 0, 0]))
                    ...
                    cntrl_point = self.train_control_points[batch_id *
                                                            self.batch_size + i]
                    cntrl_point = cntrl_point - mean.reshape((1, 1, 3))

                    if align_canonical:
                        cntrl_point = cntrl_point.reshape(
                            (self.size_u * self.size_v, 3))
                        cntrl_point = R @ cntrl_point.T
                        cntrl_point = np.reshape(
                            cntrl_point.T, (self.size_u, self.size_v, 3))

                    if anisotropic:
                        cntrl_point = cntrl_point / \
                            (std.reshape((1, 1, 3)) + EPS)
                    else:
        ...
        while True:
            for batch_id in range(self.val_size // self.batch_size - 1):
                ...
                for i in range(self.batch_size):
                    ...
                    scales.append(std)
                    Points.append(points)
                    cntrl_point = self.val_control_points[batch_id *
                                                          self.batch_size + i]
                    cntrl_point = cntrl_point - mean.reshape((1, 1, 3))
    ...
    def load_test_data(self, if_regular_points=False, align_canonical=False, anisotropic=False, if_augment=False):
        for batch_id in range(self.test_size // self.batch_size):
            ...
            for i in range(self.batch_size):
                ...
                scales.append(std)
                Points.append(points)
                cntrl_point = self.test_control_points[batch_id *
                                                       self.batch_size + i]
                cntrl_point = cntrl_point - mean.reshape((1, 1, 3))
