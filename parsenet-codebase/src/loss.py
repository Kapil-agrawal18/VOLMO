...
def spline_reconstruction_loss_one_sided(nu, nv, output, points, config, side=1):
    ...
    for b in range(config.batch_size):
        point = []
        for i in range(3):
            point.append(torch.matmul(torch.matmul(
                nu, output[b, :, :, i]), torch.transpose(nv, 1, 0)))
        reconst_points.append(torch.stack(point, 2))

    reconst_points = torch.stack(reconst_points, 0)
    reconst_points = reconst_points.view(
        config.batch_size, grid_size_u * grid_size_v, 3)
...
def uniform_knot_bspline(control_points_u, control_points_v, degree_u, degree_v, grid_size=30):
    ...
    knots_u = [0.0] * degree_u + \
        np.arange(0, 1.01, 1 / (control_points_u - degree_u)
                  ).tolist() + [1.0] * degree_u
    knots_v = [0.0] * degree_v + \
        np.arange(0, 1.01, 1 / (control_points_v - degree_v)
                  ).tolist() + [1.0] * degree_v
