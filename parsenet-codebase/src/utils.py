...
def visualize_point_cloud_from_labels(points, labels, COLORS=None, normals=None, viz=False):
    ...
    pcd = visualize_point_cloud(
        points, colors=colors, normals=normals, viz=viz)
    return pcd
...
def sample_mesh_torch(
    ...
    face_ids = np.random.choice(
        np.arange(len(areas)), size=n, p=probabilities.data.cpu.numpy())
...
def visualize_fitted_surface(output, points, grid_size, viz=True, path="data/uvmaps/"):
    ...
    for index in range(B):
        uv = output[index]
        C = np.sum(np.square(np.expand_dims(
            uv, 1) - np.expand_dims(par, 0)), 2)
        cost, x, y = lap.lapjv(C)
        p = points[index]
        p = p[y]
        fitted_surface, fitted_points = fit_surface(
            p, grid_size, grid_size, 2, 2)
        fitted_points = fitted_points - \
            np.expand_dims(np.mean(fitted_points, 0), 0)
...
def fit_surface_sample_points(output, points, grid_size, regular_grids=False):
    ...
    for index in range(B):
        ...
        p = p[y]
        fitted_surface, fitted_points = fit_surface(
            p, grid_size, grid_size, 2, 2, regular_grids)
        fitted_points = fitted_points - \
            np.expand_dims(np.mean(fitted_points, 0), 0)
        predicted_points.append(fitted_points)
...
def chamfer_distance(pred, gt, sqrt=False):
    ...
    cd = torch.mean(torch.min(diff, 1)[0], 1) + \
        torch.mean(torch.min(diff, 2)[0], 1)
